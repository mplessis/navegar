    ...
    sealed partial class App : Application
    {
        ...
        protected override void OnLaunched(LaunchActivatedEventArgs e)
        {
            ...
            if (rootFrame.Content == null)
            {
                ...
                SimpleIoc.Default.GetInstance<INavigationUwp>().InitializeRootFrame(rootFrame);
                ...
                SimpleIoc.Default.GetInstance<INavigationUwp>().PreviewNavigate += PreviewNavigate;
                SimpleIoc.Default.GetInstance<INavigationUwp>().NavigationCanceledOnPreviewNavigate += OnNavigationCanceledOnPreviewNavigate;

                //Si et seulement si il y a un bouton retour (physique ou virtuel) sur le device
                SimpleIoc.Default.GetInstance<INavigationUwp>().BackButtonPressed += BackButtonPressed;

                //Navigation vers la premiére page
                if (string.IsNullOrEmpty(SimpleIoc.Default.GetInstance<INavigationUwp>().NavigateTo<LandingPageViewModel>(true)))
                {
        ...
        {
            if (SimpleIoc.Default.GetInstance<INavigationUwp>().GetViewModelCurrent().GetType() != typeof(ListClientsPageViewModel))
            {
                //On supprime l'historique de navigation et on revient vers la page de la liste des clients (pour l'exemple)
                SimpleIoc.Default.GetInstance<INavigationUwp>().Clear();
                SimpleIoc.Default.GetInstance<INavigationUwp>().NavigateTo<ListClientsPageViewModel>(true);
            }
        }
        ...
        {
            if (SimpleIoc.Default.GetInstance<INavigationUwp>().CanGoBack())
            {
                //Lorsque l'on revient d'une commande on rafraichi la liste des commandes sur la page de liste
                if (SimpleIoc.Default.GetInstance<INavigationUwp>().GetTypeViewModelToBack() ==
                    typeof(ListCommandesPageViewModel))
                {
                    ...
                    SimpleIoc.Default.GetInstance<INavigationUwp>().GoBack("LoadDatas", new object[] { });
                }
                else
                {
                    SimpleIoc.Default.GetInstance<INavigationUwp>().GoBack();
                }
        ...
        private void OnNavigationCanceledOnPreviewNavigate(object sender, EventArgs args)
        {
            ...
            SimpleIoc.Default.GetInstance<INavigationUwp>().Clear();
            SimpleIoc.Default.GetInstance<INavigationUwp>().NavigateTo<LandingPageViewModel>(true);
        }
